...
def train(model,
    ...
    for epoch in range(epochs):
        for images, labels in train_dataloader:
            images, labels = images.to(device).to(
                torch.float), labels.to(torch.long).to(device)
...
def train_models_with_changed_params(config, model_create_func, param_range, device=None):
    if device is None:
    ...
    for param in param_range:
        ...
        train(model, opt, criterion, train_loader,
              train_loader, config.num_epochs, device)
